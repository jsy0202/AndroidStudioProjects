...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        delStudent.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    myDao.deleteEnrollmentsByStudentId(id)
        ...
        enroll.setOnClickListener {
            val studentId = editStudentId.text.toString().toInt()
            CoroutineScope(Dispatchers.IO).launch {
                // Enroll the student in class with ID 1
                myDao.insertEnrollment(Enrollment(studentId, 1))
            }
        }
    }
}